...
| AC-3 | Access Enforcement | Landlock LSM enforces filesystem ACL (irrevocable, kernel-enforced). seccomp-BPF enforces syscall restrictions. SELinux type enforcement confines agents to `puzzlepod_t` domain. All enforcement survives `puzzled` crash. |
...
| SC-3 | Security Function Isolation | `puzzled` runs in its own SELinux domain (`puzzlepod_t`). Agent processes run in `puzzlepod_t` domain. Neverallow rules prevent cross-domain access. `puzzled` is seccomp-filtered with 57 blocked escape-vector syscalls + 4 USER_NOTIF-gated syscalls (58 on x86_64). |
